...
const sortWinner = ({ percent_change_24h: pa }, { percent_change_24h: pb }) =>
  +pb - +pa

const filterNew = ({ id }) => !last.has(id)
const emoji = p =>
  p > 200 ? '⁉️🏆🏄' : p > 100 ? '🔞' : p > 50 ? '🛰' : p > 25 ? '🚀' : '🔥'
...
setInterval(() => {
  for (const [id, { expire }] of last.entries())
    if (Date.now() > expire) {
...
export default async () => {
  ...
  const cryptos = data.filter(keepWinners).filter(filterNew).map(spread)
